...
export function EditDealDialog({
  ...
  const [promptStage, setPromptStage] = useState<DealStage | null>(null);
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (stageChanged && shouldPromptLinkedBooking(stage, booking)) {
  ...
  return (
    <>
      ...
      <RequireLinkedBookingPrompt
        ...
        onOpenChange={(next) => {
          setAskBooking(next);
          if (!next) setPromptStage(null);
        }}
        dealId={deal.id}
        dealStage={promptStage}
        booking={booking}
      />
    </>
  );
